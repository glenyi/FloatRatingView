...
open class FloatRatingView: UIView {
    ...
    /// Override the caluclate margin between stars
    @IBInspectable open var imageXOffset: CGFloat = -1.0
    ...
    override open func layoutSubviews() {
        ...
        var imageXOffset: CGFloat = (frame.size.width - (imageViewSize.width * CGFloat(emptyImageViews.count))) /
            CGFloat((emptyImageViews.count - 1))
        
        if self.imageXOffset >= 0.0 {
            imageXOffset = self.imageXOffset
        }
